...
カタログ閲覧時に「7日後から6週先」の予約可能スロットを動的生成する。

> **2026-06-10更新:** 専用セッションの予約締切は**1週間前**に確定（準備フロー設計と整合）。1週間以内の枠はスロット生成から除外する。詳細は [`2026-06-10-worldclass-phase3-preparation-flow-design.md`](2026-06-10-worldclass-phase3-preparation-flow-design.md) のタイムライン参照。
...
表示項目：学校名・国・地域 / 評価スコア（★） / 空きスロット数（直近2週間）

> **2026-06-10更新:** 予約締切が1週間前のため「今週の残枠」は常にほぼ0になる。表示は「直近2週間（予約可能窓の先頭14日間）の残枠」に変更。
...
## 7. 未決定事項の確定（2026-06-10）

- **時間帯フィルタ:** 平日午前 = JST 9:00〜12:00 / 平日午後 = JST 13:00〜18:00 / 土日 = 終日
- **Stripe鍵管理:** `.env`（`stripe_secret_key` / `webhook_signing_secret`）。`.env.example` にキー名のみ記載
- **キャンセル例外（天災・回線障害）:** 運営が**手動対応**（Filamentから手動キャンセル＋手動返金トリガー）
- **確認メール:** 送信元 `no-reply@`（ドメイン確定後に設定）・Laravel Markdown Mailable（シンプルテキスト基調）
