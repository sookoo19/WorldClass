...
- [2026-07-01 全プラン横断-設計パターン](./2026-07-01_全プラン横断-設計パターン.md) — 全6プラン通読からの抽出。冪等ジョブ(送信済みカラム/一方向遷移/対象脱落)・ステータス機械+ドメイン例外・クライアント不信(金額サーバー決定/スロット再検証)・Webhook3点セット(fail-closed/冪等/CSRF除外)・仮押さえライフサイクル(draft塞ぎ+24h掃除)・lockForUpdateは数える前に本体行へ・負数decrement罠・締切計算のモデル集約・private storage+認可ルート・SVG=XSS・計画書の書き方(所有境界/BE接続契約/セルフレビュー表/先回りカラム)
...
- [2026-07-04 SlotService実装とPHPイディオム](./2026-07-04_SlotService実装とPHPイディオム.md) — SlotService🟢Green実装完了・Carbonミュータビリティ(copy()必須)・usort複合ソートキー(文字列連結+宇宙船演算子)・flip()+isset()のO(1)判定を逐行深掘り。躓き: copy()なしでstartOfDay()すると元のCarbon変数自体が書き換わる副作用
